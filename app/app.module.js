(function () {
  ...
  /* @ngInject */
  function appConfig($urlRouterProvider, $httpProvider, RestangularProvider, ENV,
    ENCOMPASSWEBCONSTANTS, $locationProvider /*,hotkeysProvider*/) {
  ...
  /* @ngInject */
  function appStart($rootScope, $state, /*encompass,applicationLoggingService, */ Restangular,
    SessionStorage, ENCOMPASSWEBCONSTANTS/*, modalService*/) {
    ...
    $rootScope.$on('$stateNotFound', function (event, toState) { // tap all stateNotFound errors and log them
      console.error('State Transition Error: event:' + event.name + ', toState:' + toState.name);
      event.preventDefault();
    });
    $rootScope.$on('$stateChangeError', function (event, toState) { // tap all stateChange errors and log them
      console.error('State Change Error: event:' + event.name + ', toState:' + toState.name);
      event.preventDefault();
    });
  }
